...
export class WebpageRecipeParserService {
    ...
    for (const script of scripts) {
      try {
        ...
        if (candidate.name || ingredients.length || instructions.length) {
          return { title: this.first(candidate.name), servings: this.first(candidate.recipeYield), ingredients, instructions, source: 'json-ld' };
        }
      } catch { /* Invalid publisher JSON-LD is ignored; text/AI fallback remains available. */ }
    }
    return undefined;
  }

  private findRecipe(value: unknown): Record<string, unknown> | undefined {
    if (Array.isArray(value)) return value.map((item) => this.findRecipe(item)).find(Boolean);
  ...
  private instructions(value: unknown): string[] {
    ...
    return value.flatMap((item) => {
      ...
      if (Array.isArray(record.itemListElement)) return this.instructions(record.itemListElement);
      return this.toStrings(record.text ?? record.name);
    }).filter(Boolean);
  }

  private toStrings(value: unknown): string[] { return Array.isArray(value) ? value.flatMap((item) => this.toStrings(item)) : typeof value === 'string' ? [value.trim()].filter(Boolean) : []; }
  private first(value: unknown): string | undefined { return this.toStrings(value)[0]; }
}
