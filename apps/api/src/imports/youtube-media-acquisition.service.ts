...
@Injectable()
export class YouTubeMediaAcquisitionService {
  constructor(private readonly config: ConfigService) {}

  async acquire(canonicalUrl: string) {
    if (!this.isEnabled()) {
      throw new ImportSourceError(
        'MEDIA_DOWNLOAD_DISABLED',
        'Video-link processing is not enabled on this server yet. Paste recipe text instead.',
      );
    }
    const root = this.config.get<string>('MEDIA_TEMP_ROOT')?.trim() || tmpdir();
    await mkdir(root, { recursive: true });
    const directory = await mkdtemp(join(root, 'pantrypal-youtube-'));
    try {
  ...
  private async download(canonicalUrl: string, directory: string) {
    ...
    const reportedPath = output
      ...
      .filter(Boolean)
      .at(-1);
    if (!reportedPath) throw new Error('Downloader did not report an output path.');
    ...
    if (
      ...
      throw new Error('Downloader returned a file outside its temporary directory.');
    }
  ...
  private run(command: string, args: string[]) {
    return new Promise<string>((resolvePromise, reject) => {
      ...
      child.once('close', (code) => {
        ...
        else reject(new Error(stderr.trim().slice(0, 400) || 'Downloader failed.'));
      });
    });
  }
  ...
  private isEnabled() {
    return (
      this.config
        .get<string>('YOUTUBE_MEDIA_DOWNLOAD_ENABLED')
        ?.trim()
        .toLowerCase() === 'true'
    );
  }
