...
export class AuthService {
  ...
  async register(input: RegisterDto) {
    ...
    const invite = await this.prisma.betaInvite.findUnique({
      where: { tokenHash: betaInviteHash },
    });
    if (!invite || invite.redeemedAt || invite.expiresAt <= new Date()) {
      throw new UnauthorizedException(
        'This beta invite is invalid, expired, or already used.',
      );
    }
    ...
    const account = await this.prisma.$transaction(async (tx) => {
      ...
      if (redeemed.count !== 1)
        throw new UnauthorizedException('This beta invite was just redeemed.');
      ...
      await tx.householdMembership.create({
        data: { householdId: household.id, accountId: created.id },
      });
      await tx.householdInviteCode.create({
        data: {
          householdId: household.id,
          codeHash: this.hashToken(householdCode),
          rotatedByAccountId: created.id,
        },
      });
      await tx.betaInvite.update({
        where: { id: invite.id },
        data: { redeemedByAccountId: created.id },
      });
  ...
  async refresh(rawRefreshToken: string) {
    // The stored credential is `<sessionId>.<raw>`; only the raw secret is
    // hashed at creation (see createSession), so verify against that portion.
    const [sessionId, rawSecret] = rawRefreshToken.split('.');
    if (!sessionId || !rawSecret)
      throw new UnauthorizedException('Your refresh session is invalid.');
    ...
    if (
      ...
      !(await argon2.verify(current.refreshTokenHash, rawSecret))
    ) {
      throw new UnauthorizedException(
        'Your refresh session is invalid or expired.',
      );
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (revoked.count !== 1)
        throw new UnauthorizedException(
          'Your refresh session was already rotated.',
        );
  ...
  async createBetaInvite(expiresInDays = 14) {
    ...
    await this.prisma.betaInvite.create({
      data: { tokenHash: this.hashToken(token), expiresAt },
    });
  ...
  private async createSession(
    ...
    const session = await client.session.create({
      data: {
        ...
        refreshTokenHash: await argon2.hash(raw, { type: argon2.argon2id }),
