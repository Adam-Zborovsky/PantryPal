...
export class AuthController {
  ...
  @Post('register')
  @ApiCreatedResponse({
    description: 'Account and first household created.',
  ...
  async refresh(
    ...
    const raw =
      client === 'web' ? request.cookies?.[refreshCookie] : input.refreshToken;
  ...
  private respondWithSession(
    ...
    if (client === 'web') {
      response.cookie(
        refreshCookie,
        result.refreshToken,
        this.cookieOptions(result.refreshExpiresAt as Date),
      );
      response.cookie(csrfCookie, randomUUID(), {
        httpOnly: false,
        sameSite: 'strict',
        secure: this.isProduction(),
      });
  ...
  private requireCsrf(request: Request, client: 'web' | 'android') {
    if (
      ...
      (!request.cookies?.[csrfCookie] ||
        request.headers['x-csrf-token'] !== request.cookies[csrfCookie])
    ) {
      throw new UnauthorizedException(
        'Your browser security token is missing or expired. Sign in again.',
      );
    }
  }
}
