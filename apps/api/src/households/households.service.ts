...
export class HouseholdsService {
  ...
  async join(accountId: string, rawCode: string) {
    const code = await this.prisma.householdInviteCode.findUnique({
      where: { codeHash: this.hash(rawCode) },
    });
    if (!code)
      throw new NotFoundException(
        'That household code is invalid. Ask a member for the current code.',
      );
    const household = await this.prisma.household.findUnique({
      where: { id: code.householdId },
    });
    if (!household)
      throw new NotFoundException('That household no longer exists.');
    const membership = await this.prisma.householdMembership.upsert({
      where: {
        householdId_accountId: { householdId: household.id, accountId },
      },
